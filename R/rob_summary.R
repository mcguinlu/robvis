...
rob_summary <- function(data, tool, overall = FALSE, weighted = TRUE,
...
# ROB-2=========================================================================
    ...
    if (tool == "ROB2") {
        ...
        if (weighted == FALSE) {
            ...
            if (NCOL(data) < 8) {
                stop("Column missing (number of columns < 8). Likely that a column detailing weights for each study is missing.")
            }
        }
        ...
        rob.tidy$domain <- factor(rob.tidy$domain, levels(rob.tidy$domain)[c(6,
            5, 4, 3, 2, 1)])

        rob.tidy$judgement <- factor(rob.tidy$judgement, levels = c("h",
            "s", "l"))

        # Create plot
        plot <- ggplot2::ggplot(data = rob.tidy) + ggplot2::geom_bar(mapping = ggplot2::aes(x = domain,
            fill = judgement, weight = Weights), width = 0.7,
            position = "fill", color = "black") + ggplot2::coord_flip(ylim = c(0,
            1)) + ggplot2::guides(fill = ggplot2::guide_legend(reverse = TRUE)) +
            ggplot2::scale_fill_manual("Risk of Bias", values = c(l = low_colour,
                s = concerns_colour, h = high_colour), labels = c(h = "  High risk of bias  ",
...
# ROBINS-I======================================================================
    ...
    if (tool == "ROBINS-I") {
        ...
        rob.tidy$domain <- factor(rob.tidy$domain, levels(rob.tidy$domain)[c(8,
            7, 6, 3, 2, 5, 4, 1)])


        rob.tidy$judgement <- factor(rob.tidy$judgement, levels = c("c",
            "s", "m", "l"))

        plot <- ggplot2::ggplot(data = rob.tidy) + ggplot2::geom_bar(mapping = ggplot2::aes(x = domain,
            fill = judgement, weight = Weights), width = 0.7,
            position = "fill", color = "black") + ggplot2::coord_flip(ylim = c(0,
            1)) + ggplot2::scale_fill_manual(values = c(m = concerns_colour,
            s = high_colour, l = low_colour, c = critical_colour),
            labels = c(c = "Critical risk  ", s = " Serious risk  ",
    ...
    if (tool == "ROBINS-I Online") {
        ...
        plot <- ggplot2::ggplot(data = rob.tidy) + ggplot2::geom_bar(mapping = ggplot2::aes(x = domain,
            ...
            ggplot2::theme(axis.title.x = ggplot2::element_blank(),
                ...
                axis.text.y = ggplot2::element_text(size = 10,
                  color = "black"), axis.line.x = ggplot2::element_line(colour = "black",
                  size = 0.5, linetype = "solid"), panel.grid.major = ggplot2::element_blank(),
                panel.grid.minor = ggplot2::element_blank(),
                panel.background = ggplot2::element_blank(),
                legend.background = ggplot2::element_rect(linetype = "solid",
                  colour = "black"), legend.title = ggplot2::element_blank(),
...
# QUADAS-2======================================================================
    ...
    if (tool == "QUADAS-2") {
        ...
        if (overall == TRUE) {
            rob.tidy$domain <- factor(rob.tidy$domain, levels(rob.tidy$domain)[c(3,
                1, 5, 2, 4)])
        } else {
            rob.tidy$domain <- factor(rob.tidy$domain, levels(rob.tidy$domain)[c(1,
                4, 2, 3)])
        }
        ...
        plot <- ggplot2::ggplot(data = rob.tidy) + ggplot2::geom_bar(mapping = ggplot2::aes(x = domain,
            ...
            position = "fill", color = "black") + ggplot2::coord_flip(ylim = c(0,
            1)) + ggplot2::guides(fill = ggplot2::guide_legend(reverse = TRUE)) +
            ggplot2::scale_fill_manual("Risk of Bias", values = c(h = high_colour,
                s = concerns_colour, l = low_colour), labels = c(h = "  High risk of bias   ",
                s = "  Some concerns      ", l = "  Low risk of bias  ")) +
            ggplot2::scale_y_continuous(labels = scales::percent) +
            ggplot2::theme(axis.title.x = ggplot2::element_blank(),
                axis.title.y = ggplot2::element_blank(), axis.ticks.y = ggplot2::element_blank(),
    ...
    if (tool == "ROB1") {
        ...
        if (weighted == FALSE) {
            ...
            if (is.numeric(data[2, ncol(data)]) == FALSE) {
                stop("Error. The final column does not seem to contain numeric values (expected for weights).")
            }
        }
